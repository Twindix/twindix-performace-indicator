...
export const ReportsView = () => {
    ...
    const recommendations = useMemo(() => {
        ...
        if (frictionScores.poorRequirements < 60) recs.push("Invest in a requirements review checklist before sprint planning to reduce rework and improve task readiness.");
        if (frictionScores.communicationGaps < 60) recs.push("Introduce a daily async standup or structured check-in to close communication gaps across the team.");
        if (frictionScores.weakOwnership < 60) recs.push("Assign a single clear owner to every task and blocker. Consider a RACI matrix for cross-functional work.");
        if (frictionScores.dependencyBlockers < 60) recs.push("Prioritize blocker resolution in daily standups. Escalate blockers older than 2 days to leadership.");
        if (frictionScores.processGaps < 60) recs.push("Standardize handoff criteria between phases. Create definition-of-done checklists for each transition.");
        if (frictionScores.teamCulture < 60) recs.push("Review workload distribution to prevent burnout. Ensure decisions are documented and shared transparently.");

        if (taskStats.completionPct < 50) recs.push("Sprint completion is below 50%. Consider reducing scope or breaking tasks into smaller deliverables.");
        if (activeBlockerCount > 3) recs.push(`There are ${activeBlockerCount} active blockers. Schedule a dedicated blocker-busting session to unblock the team.`);

        if (recs.length === 0) recs.push("The sprint is performing well across all areas. Continue current practices and look for incremental improvements.");
                ...
                <TabsContent value="summary">
                    <div className="space-y-8">
                        ...
                        <div className="text-center pb-6 border-b border-border">
                            <h2 className="text-3xl font-bold text-text-dark">{sprint?.name ?? "Current Sprint"}</h2>
                            {sprint && (
                        ...
                        <div className="flex flex-col items-center gap-4">
                            <h3 className="text-lg font-semibold text-text-dark">Overall Sprint Health</h3>
                            <ScoreGauge score={sprintMetrics?.healthScore ?? 0} size="lg" label="Health Score" />
                            ...
                            <CardContent>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                                    <div className="text-center">
                                        <p className="text-4xl font-bold text-text-dark">{taskStats.total}</p>
                                        <p className="text-sm text-text-muted mt-1">Total Tasks</p>
                                    </div>
                                    <div className="text-center">
                                        <p className="text-4xl font-bold text-success">{taskStats.completed}</p>
                                        <p className="text-sm text-text-muted mt-1">Completed</p>
                                    </div>
                                    <div className="text-center">
                                        <p className="text-4xl font-bold text-error">{taskStats.blocked}</p>
                                        <p className="text-sm text-text-muted mt-1">Blocked</p>
                                    </div>
                                    <div className="text-center">
                                        ...
                                        <p className="text-sm text-text-muted mt-1">Completion</p>
                                    </div>
                                </div>
                                <div className="mt-4 h-3 rounded-full bg-muted overflow-hidden">
                                    <div
                                        className={cn("h-full rounded-full transition-all duration-700", getBarColor(taskStats.completionPct))}
                        ...
                        {/* Key Findings */}
                        <div>
                            <h3 className="text-lg font-semibold text-text-dark mb-4">Key Findings</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {/* Strengths */}
                                {keyFindings.best.map((metric) => (
                                    <Card key={metric.id} className="border-l-4 border-l-success">
                                        <CardContent className="p-5">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div>
                                                    <p className="text-sm font-semibold text-text-dark">{metric.name}</p>
                                                    <p className="text-2xl font-bold text-success mt-1">{metric.value}{metric.unit === "%" ? "%" : ` ${metric.unit}`}</p>
                                ...
                                {keyFindings.worst.map((metric) => (
                                    <Card key={metric.id} className={cn("border-l-4", metric.status === MetricStatus.Critical ? "border-l-error" : "border-l-warning")}>
                                        <CardContent className="p-5">
                                            <div className="flex items-start gap-3">
                ...
                <TabsContent value="friction">
                    <div className="space-y-6">
                        <div className="text-center pb-4 border-b border-border">
                            <h2 className="text-2xl font-bold text-text-dark">Friction Analysis</h2>
                            <p className="text-sm text-text-secondary mt-1">
                                Detailed breakdown of the six friction areas impacting team delivery
                            </p>
                        </div>
                                    ...
                                    <CardContent className="p-6">
                                        ...
                                        <div className="flex items-start justify-between mb-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <h3 className="text-base font-semibold text-text-dark">{area.label}</h3>
                                        ...
                                        {areaMetrics.length > 0 && (
                                            <div>
                                                <p className="text-xs font-medium text-text-muted uppercase tracking-wide mb-2">Related Metrics</p>
